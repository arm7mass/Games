...
def setup():
    ...
    root = Tk()
    root.title("Artificial game of life")

    grid_view = Canvas(root, width=ch11_model.width*cell_size, height=ch11_model.height *
                       cell_size, borderwidth=0, highlightthickness=0, bg='white')
    start_button = Button(root, text='Start', width=12)
    clear_button = Button(root, text='Clear', width=12)

    choice = StringVar(root)
    choice.set('Choose a  pattern')
    option = OptionMenu(root, choice, 'Choose pf pattern',
                        'glider', 'glider gun', 'random', command=option_handler)
...
def option_handler(event):
    ...
    if selection == 'glider':
        ...
        ch11_model.randomize(ch11_model.grid_model,
                             ch11_model.width, ch11_model.height)
    update()
...
def draw_cell(row, col, color):
    ...
    grid_view.create_rectangle(row*cell_size, col*cell_size, row*cell_size +
                               cell_size, col*cell_size+cell_size, fill=color, outline=outline)
