...
def get_task():
    task = simpledialog.askstring(
        'Task', 'Do you want to encrypt or decrypt ?')
    return task
...
def encrypt(message):
    ...
    encrypted_list = []
    fake_letters = ['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i',
...
def decrypt(message):
    ...
    even_letters = get_even_letters(messagebox)
...
while True:
    ...
    if task == 'encrypt':
        ...
        messagebox.showinfo('Ciphertext of te secret message is :', encrypted)
    elif task == 'decrypt':
        message = get_message()
